...
/**
 * Servlet implementation class SearchManagerFeedServlet
 */
@WebServlet("/manager/searchfeed.do")
public class SearchManagerFeedServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchManagerFeedServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Feed> feed=new FeedManagerService().searchFeed(type,keyword,cPage,numPerpage);
		
		request.setAttribute("feed", feed);
		
		int totalData=new FeedManagerService().searchFeedCount(type, keyword);
